...
def addStudent():
    ...
    with open( os.path.join("/app/students",newFileName) , 'wb') as fh:
        fh.write(base64.decodebytes(image_data))

    student = User(id=id , username = username , classe = classe , email = email , password = password , image_file =os.path.join("/app/students",newFileName)   )
